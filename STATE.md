...
- **Isolamento de Lixo na IA e Penalidade no Motor de Recompensa**: O prompt do `AIService` foi refatorado de forma incisiva para reconhecer tags geográficas e emojis isolados como `LIXO`. Se a IA enquadrar a captura como lixo pós-inserção, o registro é imediatamente excluído do banco e os contadores de recompensa (`inserted`, `extracted`) são subtraídos, anulando o bônus para que o sistema não seja recompensado por dados corrompidos. A pontuação (`confianca_ia = 0.0`) também é zerada. O prompt também foi instruído a reconhecer falsos negativos de hostilidade política passivo-agressiva (ex: sarcasmo e acusações genéricas como "Vai trabalhar") atribuindo `is_hate = true` e mapeando-os na classe abrangente `MILICIA_DIGITAL`.
