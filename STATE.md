...
## Histórico Recente de Correções (v98.9)
   ...
   - **Orquestrador Autônomo v1.2 (Pausa Global Inteligente):** Integrado o novo mecanismo de recuperação de pausa global [sentinela_autonomous_agent.py](file:///c:/projetos/sentinela/sentinela_autonomous_agent.py). Agora, em vez de sleeps cegos de 5 minutos, o orquestrador executa verificações de 30 segundos monitorando a expiração de cooldowns dos alvos, inserções de novos alvos PENDENTES na fila Supabase ou retomada por timeout de 1 hora (resetando alvos esgotados para ativos), com alertas apropriados no Ntfy (`play_button`, `pause_button`, `alarm_clock`).
