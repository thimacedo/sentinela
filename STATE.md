...
- **Isolamento de Lixo na IA e Penalidade no Motor de Recompensa**: O prompt do `AIService` foi refatorado de forma incisiva para reconhecer tags geográficas e emojis isolados como `LIXO`. Se a IA enquadrar a captura como lixo pós-inserção, o registro é imediatamente excluído do banco e os contadores de recompensa (`inserted`, `extracted`) são subtraídos, anulando o bônus para que o sistema não seja recompensado por dados corrompidos. A pontuação (`confianca_ia = 0.0`) também é zerada. A malha de instrução foi inteiramente reestruturada com base num **módulo técnico-pedagógico avançado para detecção de incongruências**: a IA agora audita ativamente Pistas de Superfície (excesso de pontuação, aspas, risadas falsas, emojis contraditórios) e Valência Contrastante (uso de elogios e hipérboles para ofender). O rastreio engloba hostilidade passivo-agressiva, ad hominem ("só grita"), tone policing e corrupção implícita ("E o pix?") na classe `MILICIA_DIGITAL`; sarcasmo velado contra mulheres em `MISOGINIA_POLITICA`; e zombarias ou uso de tokenismo racial para validar argumentos (ex: "o sonho dele é ser branco") em `RACISMO_ESTRUTURAL`.
- **RAG (In-Context Learning) para Ironia e Sarcasmo**: Desenvolveu-se um pipeline de injeção dinâmica de contexto diretamente no `AIService`. O script `extract_pdfs.py` extrai textos de artigos científicos armazenados em `pdf_ironiaesarcasmo` para o `pasa_training_dataset.jsonl`. O `AIService` agora conta com a função `load_training_context()`, que lê esses recortes em tempo de execução e os insere como base de conhecimento diretamente no final do `SYSTEM_PROMPT` para todas as IAs em cascata (LiteRT, Ollama, Mistral, Groq), provendo "treinamento universal" de fato via In-Context Learning (limitado a 8000 caracteres para evitar estouro de tokens).
