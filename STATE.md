...
## Estabilização e Unificação do Watchdog (Fase 5)

- **Instância Única Infalível**: Implementamos uma proteção robusta baseada em socket listen na porta `8009` e um lock de boot físico (`runtime_state/watchdog_boot.lock`) com `msvcrt`/`fcntl` para eliminar race conditions de concorrência. Novas tentativas de execução em foreground ou background são abortadas instantaneamente se o watchdog já estiver rodando ou inicializando.
- **Correção de Menu Travado (Win32)**: Substituímos os callbacks de títulos dinâmicos por strings estáticas reconstruídas dinamicamente. Para evitar o bug do Win32/pystray em que os itens de menu abertos sofrem repintura e ficam em branco até passar o mouse, implementamos um comparador de tuplas de estado na thread de background: o menu só é destruído e reconstruído se houver uma alteração real em `state` (status, restarts, erros, alertas, crashes). Isso elimina por completo o bug do hover e traz fluidez e responsividade imediata (< 100ms) ao menu.
- **Unificação do Watchdog**: O watchdog unificado roda na bandeja do sistema por padrão com fallback automático para console CLI em ambientes headless (como o sandbox). A flag `--background` executa o watchdog de forma oculta sem console no Windows, sendo chamada pelo redirecionador atalho `scripts/watchdog_tray.py`.
- **Validação com Sucesso**: Iniciamos o watchdog no terminal do sandbox e comprovamos a atividade da porta de lock `8009`, enquanto segundas instâncias concorrentes foram bloqueadas e abortadas de forma imediata como projetado. Todos os patches foram commitados e empurrados para a branch `main`.
- **Tratamento de Timeouts de Conexão (Rede/Supabase)**: Corrigimos o risco de loops de reinício rápidos e desgastantes do runner sob quedas ou instabilidades de internet. Adicionamos os termos de timeout específicos do Windows (`WinError 10060`, `10060`, `timed out`, `timeout` e `componente conectado não respondeu`) à lista de verificação de falha de conexão de banco do watchdog, forçando-o a aplicar a pausa defensiva de 5 minutos antes de tentar restabelecer o ciclo de execução.
- **Tratamento Amigável de Rede no Cooldown**: Refatoramos o tratamento de exceções da manutenção interna de IA executada durante o cooldown (repouso) do Watchdog. Erros de timeout e socket offline (`WinError 10060`) agora são tratados de forma limpa e geram logs simplificados (`[Watchdog] Manutenção de IA ignorada no cooldown: Banco de dados/Rede offline.`), em vez de lançar tracebacks complexos do Windows no console e poluir a tela do dashboard.
