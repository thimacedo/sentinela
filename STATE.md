...
- **Inserção Direta de Alvos**: Inserido o perfil `@janainacpaschoal` diretamente na base via script `scratch/insert_janaina.py`, com ativação imediata na `fila_coleta`.
- **Heurística de Detecção de Lixo (Junk Filter)**: O motor V2 (`InstagramScraperV2`) foi atualizado para detectar dinamicamente elementos ofuscados da UI capturados incorretamente pelo fallback do DOM (como "Também da Meta", "Instagram Lite"). Se uma proporção crítica de lixo for detectada na varredura, o worker (`ig_worker_v2`) agora é explicitamente sinalizado com o erro `junk_detected` abortando o ciclo e poupando chamadas de API, de forma que o Worker não repita interações cegamente.
- **Isolamento de Lixo na IA**: O prompt do `AIService` foi refatorado para reconhecer fragmentos irrelevantes (LIXO). Em vez de descartá-los ruidosamente, a classificação agora zera intencionalmente o score (`confianca_ia = 0.0`) garantindo limpeza da base sem distorcer métricas da IA.
