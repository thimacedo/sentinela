...
- **Correção da Rotação de Alvos (v52.4.5)**: Diagnosticado que a `fila_coleta` (prioridade) não possuía ordenação explícita, causando o travamento da fila em alvos específicos (como @janainacpaschoal). Implementada ordenação FIFO estrita via `order("created_at", desc=False)` e adicionado um **Mecanismo de Fairness (20%)** que permite que a rotação global de candidatos avance mesmo quando a fila de prioridade está ocupada, eliminando a inanição (starvation) do pool principal.
