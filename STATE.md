...
- **Isolamento de Lixo na IA e Penalidade no Motor de Recompensa**: O prompt do `AIService` foi refatorado de forma incisiva para reconhecer tags geográficas e emojis isolados como `LIXO`. Se a IA enquadrar a captura como lixo pós-inserção, o registro é imediatamente excluído do banco e os contadores de recompensa (`inserted`, `extracted`) são subtraídos, anulando o bônus para que o sistema não seja recompensado por dados corrompidos. A pontuação (`confianca_ia = 0.0`) também é zerada. O prompt também foi estendido para mapear hostilidade política passivo-agressiva (ex: "Vai trabalhar") em `MILICIA_DIGITAL`, e sarcasmo velado focado em mulheres (ex: "meninas do Valdemar", "descontrolada") em `MISOGINIA_POLITICA`.
