...
- **Integração Total de Anúncios e Placeholder Visual (v84.11)**: Removidas tags AMP conflitantes do `layout.tsx`. O componente `AdSenseSlot.tsx` foi refatorado para ter altura mínima estrita e um placeholder visual dinâmico ("Publicidade"), impedindo que os espaços fiquem "vazios" (colapsados) quando bloqueadores de anúncio estão ativos ou em localhost. Anúncios expandidos e integrados para todas as rotas secundárias (`/analise`, `/rede`, `/dossies`, `/relatorios`).
