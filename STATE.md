...
- Refatorado o script `"dev"` no `package.json` da raiz do projeto para iniciar o Next.js local em modo de desenvolvimento (`npm run dev --prefix frontend`) na porta 3000 em vez de servir arquivos estáticos legados via `http-server`, garantindo atualizações de interface e dados em tempo real (Fase 4 - executado por Claude Sonnet 4.6 (Thinking)).
- Removido o limite fixo de extração de 3 posts em `core/instagram_headless.py` e `workers/scrapers/ig_headless.py`, tornando-o dinâmico e parametrizável através da variável global `MAX_POSTS_PER_PROFILE` (ou do parâmetro `max_posts` repassado da config do worker), o que permite a flexibilização total da paginação de posts de acordo com a variável de ambiente correspondente (Fase 4 - executado por Claude Sonnet 4.6 (Thinking)).
