...
- **Isolamento de Lixo na IA e Penalidade no Motor de Recompensa**: O prompt do `AIService` foi refatorado de forma incisiva para reconhecer tags geográficas e emojis isolados como `LIXO`. Se a IA enquadrar a captura como lixo pós-inserção, o registro é imediatamente excluído do banco e os contadores de recompensa (`inserted`, `extracted`) são subtraídos, anulando o bônus para que o sistema não seja recompensado por dados corrompidos. A pontuação (`confianca_ia = 0.0`) também é zerada. O prompt também foi estendido agressivamente para mapear hostilidade política passivo-agressiva, ad hominem ("só grita", "esquisito"), tone policing ("se falar com calma"), acusações de corrupção ("E o pix?") e associação criminosa ("black bloc") na classe abrangente `MILICIA_DIGITAL`. Além disso, sarcasmo velado focado em mulheres (ex: "meninas do Valdemar", "descontrolada") recai em `MISOGINIA_POLITICA`, e zombarias, invalidação de traços raciais ou uso de tokenismo racial para validar argumentos (ex: "o sonho dele é ser branco", "muitos negros que pensam assim") em `RACISMO_ESTRUTURAL`.
